...
This file provides guidance to Codex when working in a Fallout: New Vegas modding environment. It is the Codex-facing twin of `CLAUDE.md`: the FNV knowledge is shared, and the agent-integration sections (MCP servers, skills, safety hooks) are written for Codex's conventions.
...
> **This install is managed by Mod Organizer 2 (MO2).** Open Codex in the **game root** (project dir), but understand that **your mods do NOT live in `Data/`** — they live in the MO2 instance. Read "Mod Organizer 2 Layout" below before touching anything mod-related.
...
**Consequences for Codex — internalize these:**
...
- **New files Codex generates that the game must load** (via MO2) belong in `overwrite/` or a dedicated mod folder under `mods/`, then enabled in MO2 — not dropped into `Data/`.
...
YesMan AI includes a Mod Organizer 2 plugin (`mo2-mcp/`), installed with the toolbox by the YesMan AI installer (registered in Codex's `config.toml` as an MCP server). When MO2 is open with it enabled, the session has the **mo2 MCP tools** (`mo2_list_mods`, `mo2_query_records`, `mo2_conflict_summary`, `mo2_create_patch`, …) that give live MO2 awareness the CLI can't: the virtual file system, the active load order, cross-plugin conflict/override analysis, and record reading on the **modded** order.

**Detect-and-prefer rule.** The plugin ships with the toolbox, but it only *runs* while MO2 is open. If the mo2 MCP tools are present in the session → **prefer them** for the tasks below. If they're absent (MO2 closed, or a non-MO2 / Vortex / manual setup), **fall back to the AutoMod CLI / through-MO2** — every workflow still works without the MCP. **Never assume it's running; never require it at runtime.**
...
Codex `PreToolUse` hooks (configured in `.codex/config.toml` / `hooks.json`, installed with the toolbox) enforce these automatically — they run before a Bash command or an `apply_patch` file edit and can **deny** it outright or **ask** for confirmation.
...
### Audit trail
- Every file edit is auto-backed up to `.codex/backups/` with a timestamp.
- An audit log at `.codex/backups/AUDIT_LOG.txt` records every file touched, when, and by which tool.
