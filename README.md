...
- **An ESP backbone that works** — programmatic read/edit/diff of FNV plugins from Node via **xEditLib** (`GM_FNV=0`), validated against real game files.
- **Writes GECK scripts _and_ native plugins** — from in-plugin GECK scripts (`SCPT`/`SCTX`, NVSE — the real FNV model, not Papyrus) all the way up to complete **xNVSE script-extender plugins**: with Visual Studio installed, the `build` module and `/nvse-plugin` skill scaffold the C++ and compile a working 32-bit `.dll` (validated in-game).
